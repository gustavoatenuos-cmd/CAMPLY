...
// 1. Imports
if (!ov.includes('import { LayoutGrid, List } from \'lucide-react\';')) {
  ov = "import { LayoutGrid, List } from 'lucide-react';\n" + ov;
}
if (!ov.includes('ClientPerformanceCardGrid')) {
  ov = ov.replace(
...
// 2. State
if (!ov.includes('camply:dashboard-view-mode')) {
  ov = ov.replace(
...
// 3. Render
if (ov.includes("<ClientPerformanceTable clients={sortedClients} period={period} />")) {
  ov = ov.replace(
