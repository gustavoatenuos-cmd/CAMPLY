    ...
    <section className="h-full overflow-y-auto bg-brand-ink p-4 sm:p-5 lg:p-8">
      <div className="mx-auto max-w-[1700px] space-y-6">
        <header className="glass-panel flex animate-fade-in flex-col gap-4 rounded-2xl p-5 sm:flex-row sm:items-end sm:justify-between lg:p-6">
          ...
          <button type="button" onClick={() => { setEditingId(null); setModalOpen(true); }} className="btn-primary shrink-0"><Plus size={17} /> Nova campanha operacional</button>
        </header>

        <MetaOperationalWorkspace data={data} compact />

        <section className="glass-panel animate-fade-in rounded-2xl p-5 lg:p-6">
          <div className="flex flex-col gap-1 sm:flex-row sm:items-end sm:justify-between">
            ...
            {operationalCampaigns.length > 0 && (
              <p className="shrink-0 rounded-full border border-brand-line px-3 py-1 text-xs font-bold text-brand-muted">{operationalCampaigns.length} campanha{operationalCampaigns.length !== 1 ? 's' : ''}</p>
            )}
          </div>
          <div className="mt-5 grid gap-4 md:grid-cols-2 xl:grid-cols-3">
            {operationalCampaigns.map((campaign) => {
              const client = data.clients.find((item) => item.id === campaign.clientId);
              const priorityStyle = campaign.priority === 'high'
                ? 'text-rose-300'
                : campaign.priority === 'medium' ? 'text-amber-300' : 'text-brand-soft';
              const priorityLabel = campaign.priority === 'high' ? 'Alta' : campaign.priority === 'medium' ? 'Média' : 'Baixa';
              return (
                <article key={campaign.id} className="glass-card group rounded-xl p-4">
                  <div className="flex items-start justify-between gap-3">
                    <div className="min-w-0"><p className="text-xs font-bold text-brand-green">{campaign.platform}</p><h3 className="mt-1 truncate font-black text-white">{campaign.name}</h3><p className="mt-1 truncate text-xs text-brand-muted">{clientDisplayName(client)}</p></div>
                    <button type="button" aria-label={`Editar ${campaign.name}`} onClick={() => { setEditingId(campaign.id); setModalOpen(true); }} className="shrink-0 rounded-lg border border-brand-line p-2 text-brand-soft opacity-70 transition hover:border-brand-green/40 hover:text-brand-green group-hover:opacity-100 focus-visible:opacity-100"><Edit3 size={14} /></button>
                  </div>
                  <div className="mt-4 grid grid-cols-2 gap-2"><div className="rounded-lg bg-white/[0.03] p-3"><p className="text-[10px] uppercase text-brand-muted">Verba planejada</p><p className="mt-1 font-black text-white">{money(campaign.budget)}</p></div><div className="rounded-lg bg-white/[0.03] p-3"><p className="text-[10px] uppercase text-brand-muted">Prioridade</p><p className={`mt-1 font-black ${priorityStyle}`}>{priorityLabel}</p></div></div>
                  <label className="mt-3 block text-xs font-bold text-brand-soft">Etapa operacional<select value={campaign.status} onChange={(event) => setStatus(campaign, event.target.value as CampaignStatus)} className="input-field mt-1">{campaignColumns.map((status) => <option key={status} value={status}>{campaignStatusLabels[status]}</option>)}</select></label>
          ...
          {operationalCampaigns.length === 0 && (
            <div className="mt-5">
              <EmptyState
                icon={Megaphone}
                title="Nenhuma campanha operacional criada"
                description="As campanhas oficiais da Meta continuam visíveis acima. Use o quadro para organizar o planejamento interno."
                action={<button type="button" onClick={() => { setEditingId(null); setModalOpen(true); }} className="btn-primary"><Plus size={15} /> Criar a primeira</button>}
              />
            </div>
          )}
        </section>
        ...
        <Modal title={editing ? 'Editar campanha operacional' : 'Nova campanha operacional'} description="Planejamento interno sem copiar métricas da plataforma." open={modalOpen} onClose={() => { setModalOpen(false); setEditingId(null); }}>
          <form key={editing?.id || 'new'} onSubmit={saveCampaign} className="space-y-4 p-5">
            <div className="grid gap-4 md:grid-cols-2">
              ...
              <label className="text-sm font-bold text-brand-soft">Cliente<select name="clientId" defaultValue={editing?.clientId || ''} required className="input-field mt-2"><option value="">Selecione</option>{data.clients.map((client) => <option key={client.id} value={client.id}>{clientOptionLabel(client, data.projects)}</option>)}</select></label>
              <label className="text-sm font-bold text-brand-soft">Plataforma<select name="platform" defaultValue={editing?.platform || 'Meta Ads'} className="input-field mt-2">{campaignPlatforms.map((platform) => <option key={platform}>{platform}</option>)}</select></label>
              <label className="text-sm font-bold text-brand-soft">Objetivo<select name="objective" defaultValue={String(editing?.objective || 'Tráfego')} className="input-field mt-2">{metaCampaignObjectives.map((objective) => <option key={objective}>{objective}</option>)}</select></label>
              <Field label="Verba planejada" name="budget" type="number" min="0" step="0.01" defaultValue={editing?.budget || 0} />
              <label className="text-sm font-bold text-brand-soft">Prioridade<select name="priority" defaultValue={editing?.priority || 'medium'} className="input-field mt-2"><option value="low">Baixa</option><option value="medium">Média</option><option value="high">Alta</option></select></label>
              <label className="text-sm font-bold text-brand-soft">Etapa<select name="status" defaultValue={editing?.status || 'setup'} className="input-field mt-2">{campaignColumns.map((status) => <option key={status} value={status}>{campaignStatusLabels[status]}</option>)}</select></label>
              <Field label="Próxima ação" name="nextAction" defaultValue={editing?.nextAction} />
            </div>
            <div className="flex justify-end gap-2 border-t border-brand-line pt-4"><button type="button" onClick={() => setModalOpen(false)} className="btn-ghost">Cancelar</button><button className="btn-primary">Salvar</button></div>
          </form>
        </Modal>
      </div>
    </section>
...
function Field({ label, name, ...props }: InputHTMLAttributes<HTMLInputElement> & { label: string; name: string }) {
  return <label className="text-sm font-bold text-brand-soft">{label}<input name={name} className="input-field mt-2" {...props} /></label>;
}
