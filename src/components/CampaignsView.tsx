    ...
    <section className="flex h-full flex-col">
      <header className="border-b border-slate-800 bg-slate-950 px-6 py-5">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
          <div>
            <p className="text-sm font-semibold uppercase tracking-wider text-emerald-400">Campanhas</p>
            <h1 className="mt-1 text-2xl font-black text-white">Quadro operacional</h1>
          </div>
          <button onClick={addCampaign} className="inline-flex items-center justify-center gap-2 rounded-lg bg-emerald-400 px-4 py-3 text-sm font-bold text-slate-950">
            <Plus size={18} />
            Nova campanha
          </button>
        </div>
        ...
        <DragDropContext onDragEnd={onDragEnd}>
          <div className="flex h-full min-w-max gap-4">
            {campaignColumns.map((status) => {
              ...
              return (
                <div key={status} className="flex h-full w-[300px] flex-col rounded-xl border border-slate-800 bg-slate-950">
                  <div className="flex items-center justify-between border-b border-slate-800 p-4">
                    <h2 className="text-sm font-bold text-white">{campaignStatusLabels[status]}</h2>
                    <span className="rounded-full bg-slate-900 px-2 py-1 text-xs text-slate-400">{cards.length}</span>
                  </div>
                            ...
                            <Draggable key={campaign.id} draggableId={campaign.id} index={index}>
                              {(dragProvided, snapshot) => (
                                <article
                                  ...
                                  className={`rounded-lg border border-slate-800 bg-slate-900 p-4 ${snapshot.isDragging ? 'border-emerald-400' : ''}`}
                                >
                                  <p className="text-xs text-slate-500">{client?.name}</p>
                                  <h3 className="mt-1 font-bold text-white">{campaign.name}</h3>
                                  <p className="mt-2 line-clamp-2 text-sm text-slate-400">{campaign.nextAction}</p>
                                  <div className="mt-4 flex items-center justify-between text-xs text-slate-500">
                                    <span>{campaign.platform}</span>
                                    <span className={campaign.priority === 'high' ? 'text-rose-400' : 'text-emerald-400'}>{campaign.priority}</span>
                                  </div>
