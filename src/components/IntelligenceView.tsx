...
export function IntelligenceView({ data, insights }: IntelligenceViewProps) {
  ...
  return (
    <section className="h-full overflow-y-auto p-6 lg:p-8">
      <div className="mb-6">
        <p className="text-sm font-semibold uppercase tracking-wider text-emerald-400">Inteligência Camply</p>
      ...
      <div className="space-y-4">
        {insights.map((insight) => (
          <article key={insight.id} className={`rounded-xl border bg-slate-950 p-5 ${borderFor(insight.level)}`}>
            <div className="flex gap-4">
              <div className={`mt-1 rounded-lg p-2 ${iconFor(insight.level)}`}>
                <AlertTriangle size={20} />
              </div>
              <div>
                <p className="text-xs font-semibold uppercase tracking-wider text-slate-500">Recomendação</p>
                <h2 className="mt-1 text-lg font-bold text-white">{insight.title}</h2>
                <p className="mt-2 leading-relaxed text-slate-400">{insight.description}</p>
                <p className="mt-4 rounded-lg bg-slate-900 p-3 text-sm font-semibold text-emerald-400">{insight.recommendation}</p>
              </div>
            </div>
          </article>
        ))}
...
const borderFor = (level: Insight['level']) => {
  ...
  if (level === 'good') return 'border-emerald-400/40';
...
const iconFor = (level: Insight['level']) => {
  ...
  if (level === 'good') return 'bg-emerald-400/10 text-emerald-400';
