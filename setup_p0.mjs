...
// Fix agentAlerts assignment with transitions
const syncAppLogic = `...(() => {
...
app = app.replace(/agentAlerts:\s*syncOperationalSignals\(\s*current\.agentAlerts,\s*evaluateOperationalSignals\([^)]+\)\s*\),?/g, syncAppLogic);

// second call (with empty clients etc)
const syncAppLogic2 = `...(() => {
              const syncResult = syncOperationalSignals(
                current.agentAlerts,
                evaluateOperationalSignals($1)
              );
              return { agentAlerts: syncResult.signals };
            })(),`;
app = app.replace(/agentAlerts:\s*syncOperationalSignals\(\s*current\.agentAlerts,\s*evaluateOperationalSignals\(([^)]+)\)\s*\),?/g, syncAppLogic2);
...
// Fix fixtures using raw objects in previousSignals block
...
// Fix indexing error because test tests the evaluator but passes it to sync
testCode = testCode.replace(/expect\(syncOperationalSignals\(evaluateOperationalSignals\(([^)]+)\)\)\.signals\[0\]/g, "expect(evaluateOperationalSignals($1)[0]");
