-- Global dashboard metrics now roll up the newest daily fact per date across
-- sync runs. The last_90d run remains a structure/reference base only.
...
BEGIN
  ...
  account_freshness AS (
    SELECT
      a.client_meta_asset_id,
      a.client_id,
      max(f.fact_date) AS fresh_through,
      max(f.source_finished_at) AS refreshed_at
    FROM accounts a
    LEFT JOIN private.meta_daily_metric_facts f
      ON f.user_id = v_user_id
     AND f.integration_id = a.integration_id
     AND f.ad_account_id = a.ad_account_id
     AND f.source_level = 'account'
    GROUP BY a.client_meta_asset_id, a.client_id
  ),
  account_metric_sums AS (
    SELECT
      ...
      max(m.source_finished_at) AS finished_at,
      m.metric_id,
    ...
    JOIN private.meta_daily_metric_facts m
     ...
     AND m.fact_date BETWEEN sr.date_start AND sr.date_stop
    ...
    SELECT
      ...
      sr.date_stop,
      ls.id AS sync_run_id,
      COALESCE(af.refreshed_at, ls.finished_at) AS finished_at,
      sm.metric_id,
      0::numeric AS raw_metric_value,
      'zero_delivery' AS completeness_status
    FROM latest_success ls
    JOIN selected_ranges sr ON sr.client_meta_asset_id = ls.client_meta_asset_id
    LEFT JOIN account_freshness af ON af.client_meta_asset_id = ls.client_meta_asset_id
      ...
      COALESCE(jsonb_object_agg(
        ...
        public.decorate_analytics_metric(
          ...
          jsonb_build_object(
            ...
            'syncRunId', ls.id,
            'collectedAt', COALESCE(amv.finished_at, ls.finished_at),
  ...
  group_metric_sums AS (
    ...
    LEFT JOIN public.meta_campaign_entities cs
      ON cs.user_id = v_user_id
     AND cs.integration_id = ls.integration_id
     AND cs.ad_account_id = ls.ad_account_id
     AND cs.campaign_id = m.campaign_id
    LEFT JOIN public.meta_adset_entities ads
      ON ads.user_id = v_user_id
     AND ads.integration_id = ls.integration_id
     AND ads.ad_account_id = ls.ad_account_id
          ...
          CASE
            ...
            WHEN EXISTS (
              ...
              WHERE af.client_id = ac.client_id
                AND (
                  af.fresh_through IS NULL
                  OR af.fresh_through < sr.date_stop
                  OR af.refreshed_at IS NULL
        ...
        'accounts', COALESCE((
          SELECT jsonb_agg(
            jsonb_build_object(
              ...
              'freshThrough', af.fresh_through,
              'refreshedAt', af.refreshed_at,
          ...
          LEFT JOIN account_freshness af ON af.client_meta_asset_id = a.client_meta_asset_id
