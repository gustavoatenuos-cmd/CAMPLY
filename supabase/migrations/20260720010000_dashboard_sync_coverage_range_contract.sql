-- Use date coverage, not requested_period equality, for dashboard sync evidence.
-- A larger successful run (for example last_90d) can cover smaller operational
-- reads (today, this_week, last_7d, last_30d, this_month) when normalized
-- metrics exist at daily/sufficient granularity inside the selected range.
...
BEGIN
  ...
  selected_ranges AS (
    SELECT
      ...
      CASE
        WHEN p_period = 'this_month' THEN date_trunc('month', local_today)::date
        WHEN p_period = 'this_week' THEN (local_today - ((((extract(dow from local_today)::int + 6) % 7)) * interval '1 day'))::date
