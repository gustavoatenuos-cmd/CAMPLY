    ...
    jsonb_agg(
      jsonb_build_object(
        ...
        'accounts', COALESCE((
          SELECT jsonb_agg(
            jsonb_build_object(
              ...
              'dataQuality', jsonb_build_object(
                'status', CASE
                  WHEN uar.id IS NULL THEN 'unavailable'
                  WHEN COALESCE(amj.has_partial, false) THEN 'partial'
                  WHEN la.status IN ('partial', 'failed') AND la.started_at > uar.started_at THEN 'partial'
                  WHEN uar.status = 'partial' THEN 'partial'
                  ELSE 'complete'
                END,
                'reason', CASE
                  WHEN uar.id IS NULL THEN 'no_usable_run'
                  WHEN COALESCE(amj.has_partial, false) THEN amj.partial_reason
                  WHEN la.status IN ('partial', 'failed') AND la.started_at > uar.started_at THEN COALESCE(la.termination_reason, la.error_message, 'newer_incomplete_attempt')
                  WHEN uar.status = 'partial' THEN 'usable_partial_run'
                  ELSE NULL
                END
              ),
              'dataQualityByScope', jsonb_build_object(
                'account', jsonb_build_object(
                  'status', COALESCE(uar.account_coverage_status, CASE WHEN uar.status = 'success' THEN 'complete' ELSE 'unavailable' END),
                  'reason', CASE WHEN uar.id IS NULL THEN 'no_usable_run' ELSE NULL END
                ),
                'campaigns', jsonb_build_object(
                  'status', COALESCE(ucr.campaigns_coverage_status, CASE WHEN ucr.status = 'success' THEN 'complete' ELSE 'unavailable' END),
                  'reason', CASE WHEN ucr.id IS NULL THEN 'no_usable_run' ELSE NULL END
                )
              ),
              'dataRun', CASE WHEN uar.id IS NULL THEN NULL ELSE jsonb_build_object(
                'id', uar.id,
                'status', uar.status,
                'startedAt', uar.started_at,
                'finishedAt', uar.finished_at,
                'terminationReason', uar.termination_reason,
                'requestedPeriod', uar.requested_period,
                'runScope', uar.run_scope,
                'dateStart', uar.date_start,
                'dateStop', uar.date_stop
              ) END,
          ...
          LEFT JOIN selected_ranges sr ON sr.client_meta_asset_id = a.client_meta_asset_id
          LEFT JOIN usable_account_run uar ON uar.client_meta_asset_id = a.client_meta_asset_id
          LEFT JOIN usable_campaign_run ucr ON ucr.client_meta_asset_id = a.client_meta_asset_id
          LEFT JOIN latest_success ls ON ls.client_meta_asset_id = a.client_meta_asset_id
