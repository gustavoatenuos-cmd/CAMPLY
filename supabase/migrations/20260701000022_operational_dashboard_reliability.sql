-- Migration 000022: operational dashboard reliability and client registry repair.
-- Explicitly replaces the public dashboard contract with direct definitions
-- and repairs client_identity from the workspace JSON.

BEGIN;

WITH workspace_clients AS (
  SELECT
    w.id::uuid AS user_id,
    btrim(client->>'id') AS client_id,
    COALESCE(
      NULLIF(btrim(client->>'company'), ''),
      NULLIF(btrim(client->>'name'), ''),
      NULLIF(btrim(client->>'id'), '')
    ) AS display_name
  FROM public.camply_workspace w
  CROSS JOIN LATERAL jsonb_array_elements(COALESCE(w.data->'clients', '[]'::jsonb)) AS client
  WHERE w.id ~* '^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$'
    AND NULLIF(btrim(client->>'id'), '') IS NOT NULL
)
INSERT INTO public.client_identity (user_id, client_id, display_name, archived_at)
SELECT user_id, client_id, display_name, NULL
...
DO $smoke$
BEGIN
  IF to_regprocedure('public.get_analytics_capabilities()') IS NULL THEN
    RAISE EXCEPTION 'get_analytics_capabilities was not created';
  END IF;

  IF to_regprocedure('public.decorate_analytics_metric(text,jsonb,jsonb)') IS NULL THEN
    RAISE EXCEPTION 'decorate_analytics_metric was not created';
  END IF;

  IF to_regprocedure('public.get_global_performance_dashboard_v2(text,text[],uuid[])') IS NULL THEN
    RAISE EXCEPTION 'get_global_performance_dashboard_v2 was not created';
  END IF;
  ...
  IF NOT EXISTS (
    ...
    WHERE table_schema = 'public'
      AND table_name = 'client_identity'
  ) THEN
    RAISE EXCEPTION 'client_identity is missing';
  END IF;
END;
$smoke$;
