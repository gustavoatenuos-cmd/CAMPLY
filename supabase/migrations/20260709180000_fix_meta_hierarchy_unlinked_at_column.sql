-- Fix: get_meta_performance_hierarchy referenced a non-existent "deleted_at"
-- column on client_meta_assets. The table's soft-unlink column is
-- "unlinked_at" (see 20260627000015_multiclient_performance_foundation.sql
-- and every other query against client_meta_assets in this codebase). This
-- broke every call to the RPC with "column \"deleted_at\" does not exist".
...
BEGIN
  ...
  WHERE client_meta_asset_id = v_link.id
    AND user_id = v_user_id
    AND period = p_period
  ...
  IF p_level = 'campaign' THEN
    ...
    WHERE sync_run_id = v_run.id
      ...
      AND (
        ...
        OR (
          COALESCE(NULLIF(upper(effective_status), ''), NULLIF(upper(meta_status), ''), '') = 'ACTIVE'
          AND (
            NOT EXISTS (
              SELECT 1
  ...
  RETURN jsonb_build_object(
    ...
    'dateStart', (v_run.range_diagnostics_by_period->p_period->'account'->>'dateStart')::text,
    'dateStop', (v_run.range_diagnostics_by_period->p_period->'account'->>'dateStop')::text,
