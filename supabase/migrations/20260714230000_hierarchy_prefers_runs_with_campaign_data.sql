-- 20260714220000 widened run selection to status IN ('success','partial') ordered
-- by recency, to fix a stale 12-day-old 'success' run shadowing a newer 'partial'
-- run that had already written 100 real campaign snapshots. That's correct when
-- the newer run actually wrote data - but a 'partial' run can also terminate
-- before writing anything for this level (e.g. failed on the very first Graph API
-- page). Ordering by recency alone then picks that empty run over an older run
-- that has real snapshots, showing an empty hierarchy instead of the last good data.
-- Caught by supabase/tests/meta_analytics_smoke.sql, which seeds exactly this
-- shape: a newer 'partial' run with zero meta_campaign_snapshots rows alongside
-- an older 'success' run that has them.
--
-- Fix: only consider a candidate run if it actually wrote at least one campaign
-- snapshot. Recency among those with data is unchanged from 20260714220000.
--
-- Body otherwise unchanged from 20260714220000_hierarchy_accepts_partial_runs.sql.
