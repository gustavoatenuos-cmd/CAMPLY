-- Campaign drill-down also lists paused campaigns that delivered in the period.
--
-- The campaign level of get_meta_performance_hierarchy only returned campaigns
-- whose current status is ACTIVE. A campaign paused yesterday that spent in the
-- last 30 days vanished from the drill-down and from the "Campanhas Meta" board,
-- while its spend still counted in the client's account totals.
--
-- Campaign level now returns ACTIVE campaigns plus any campaign with spend > 0
-- in the slice (legacy exact-period reads have no slice: ACTIVE only, as before).
-- Ordering and the rest of the body are unchanged from 20260922010000.
