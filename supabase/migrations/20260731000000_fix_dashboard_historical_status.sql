-- Fix dashboard historical analytics to include campaigns with delivery regardless of current status
  ...
  group_metric_sums AS (
    ...
    WHERE EXISTS (SELECT 1 FROM supported_metric_ids sm WHERE sm.metric_id = m.metric_id)
      AND (
        ...
        OR EXISTS (
          SELECT 1 FROM public.meta_normalized_metrics md
          WHERE md.sync_run_id = m.sync_run_id
            AND md.campaign_id = m.campaign_id
            AND md.source_level = 'campaign'
            AND md.metric_id IN ('spend', 'impressions')
            AND md.metric_value > 0
        )
      )
    GROUP BY
      ls.client_id,
    ...
    jsonb_agg(
      jsonb_build_object(
            ...
            jsonb_build_object(
              ...
              'lastSuccessfulRun', CASE WHEN ls.id IS NULL THEN NULL ELSE jsonb_build_object(
                ...
                'metricsCount', (
                  ...
                  WHERE m.sync_run_id = ls.id
                ),
                'metricGroupsCount', (
                  SELECT count(DISTINCT (m.campaign_id, m.attribution_setting))::int
                  FROM public.meta_normalized_metrics m
                  WHERE m.sync_run_id = ls.id
                    AND m.campaign_id IS NOT NULL
                )
        ...
        'resolvedTargets', COALESCE((
          ...
          WHERE t.client_id = ac.client_id
        ), '[]'::jsonb),
        'evaluations', '[]'::jsonb,
        'budgetPacing', NULL,
        ...
        'dataQuality', jsonb_build_object(
          ...
          'reason', CASE
            ...
            WHEN EXISTS (
              ...
              SELECT COALESCE(la.termination_reason, la.error_message, 'newer_incomplete_attempt')
        ...
        'lastSuccessfulRun', (
          ...
          FROM latest_success ls
          WHERE ls.client_id = ac.client_id
          ORDER BY ls.finished_at DESC NULLS LAST
          LIMIT 1
        ),
        'lastAttempt', (
          SELECT CASE WHEN la.id IS NULL THEN NULL ELSE jsonb_build_object(
            'id', la.id,
            'status', la.status,
            'startedAt', la.started_at,
            'finishedAt', la.finished_at,
