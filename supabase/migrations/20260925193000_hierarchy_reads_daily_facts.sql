-- Read hierarchy structure from the newest usable run and metrics from the
-- cross-run daily freshness read model. This lets today's lightweight sync
-- update 7/30/90-day reads without rebuilding the whole historical run.

-- Expose the number of currently ACTIVE campaigns separately from the broader
-- hierarchy total (ACTIVE + paused campaigns that delivered in the period).
--
-- The generic analytics hierarchy keeps its historical delivery behavior. The
-- operational campaigns board uses activeTotal to load and display only current
-- ACTIVE campaigns without confusing historical delivery with active inventory.
...
BEGIN
  ...
  IF v_is_slice THEN
    ...
    -- Structure and metrics are intentionally decoupled. The newest usable
    -- full-account structure can come from today's lightweight refresh, while
    -- period metrics are rolled up from private.meta_daily_metric_facts.
    SELECT r.* INTO v_run
    ...
    WHERE r.user_id = v_user_id
      ...
      AND (
        p_level NOT IN ('ad', 'creative')
        OR EXISTS (
          SELECT 1 FROM public.meta_ad_snapshots ads
          WHERE ads.sync_run_id = r.id AND ads.user_id = v_user_id
        )
      )
  ...
  IF p_level = 'campaign' THEN
    ...
    FROM (
      SELECT cs.effective_status, cs.meta_status,
        (
          SELECT SUM(m.metric_value)
          FROM private.meta_daily_metric_facts m
          WHERE v_date_start IS NOT NULL
            AND m.user_id = v_user_id
            AND m.integration_id = v_link.integration_id
            AND m.ad_account_id = v_link.ad_account_id
            AND m.source_level = 'campaign'
            AND m.campaign_id = cs.campaign_id
            AND m.adset_id IS NULL
            AND m.ad_id IS NULL
            AND m.metric_id = 'spend'
            AND m.fact_date BETWEEN v_date_start AND v_date_stop
        ) AS slice_spend
      FROM public.meta_campaign_snapshots cs
      WHERE cs.sync_run_id = v_run.id
        AND cs.user_id = v_user_id
    ) c
    WHERE COALESCE(NULLIF(upper(c.effective_status), ''), NULLIF(upper(c.meta_status), ''), '') = 'ACTIVE'
       OR COALESCE(c.slice_spend, 0) > 0;
    ...
    WHERE COALESCE(NULLIF(upper(s0.effective_status), ''), NULLIF(upper(s0.meta_status), ''), '') = 'ACTIVE'
       OR COALESCE(s0.slice_spend, 0) > 0
    ORDER BY s0.slice_spend DESC NULLS LAST, s0.campaign_name OFFSET v_offset LIMIT p_page_size
    ) s;
  ...
  RETURN jsonb_build_object(
    ...
    'level', p_level, 'period', p_period, 'page', p_page, 'pageSize', p_page_size,
    'total', v_total, 'activeTotal', CASE WHEN p_level = 'campaign' THEN v_active_total ELSE NULL END, 'items', v_items,
    'clientId', v_link.client_id, 'clientMetaAssetId', v_link.id,
    'metaAssetId', v_link.meta_asset_id, 'integrationId', v_link.integration_id,
    'adAccountId', v_link.ad_account_id,
